...
for x in data_tiles:
    ...
    print(x)
    print( os.path.exists(dtm_path),os.path.exists(hat_path),os.path.exists(sobel_path))
...
vrt_1km = glob.glob(out_dir + '/*.vrt')
vrt_10km_dir = 'V:/2022-03-31_Stendiger_EZRA/training_data/initial_area/dem/vrt10km_pilotarea/'

print(len(vrt_1km))
#%%
km1_pilot = gpd.read_file(r'V:\2022-03-31_Stendiger_EZRA\stonewalls_slks\data\pilot_training_data\Pilot_1km_grid.gpkg')
km10_pilot = gpd.read_file(r'V:\2022-03-31_Stendiger_EZRA\stonewalls_slks\data\pilot_training_data\Pilot_10km_grid.gpkg')
...
for i,x in enumerate(dict_tiles.keys()):
    ...
    print(f"{x} tile with {tilename} bounds with {len(dict_tiles[x])} 1km tiles")

    vrt = vrt_10km_dir + x + '.vrt'

    print(vrt)
    src = [out_dir + y +'.vrt' for y in dict_tiles[x]]
    # for t in dict_tiles[x]:
    #     if not os.path.isfile(out_dir + t +'.vrt'):
    #         km1_missing_list.append(t)
    #     else: continue
    # print(src)
    gdal.BuildVRT(vrt, src, options=gdal.BuildVRTOptions(outputBounds=(xmin, ymin, xmax, ymax)))
