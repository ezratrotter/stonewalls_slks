...
sys.path.append(yellow_path)
sys.path.append(yellow_path + 'buteo/')
sys.path.append(yellow_path + 'buteo/machine_learning/')
sys.path.append(yellow_path + 'buteo/filters/')
sys.path.append(yellow_path + 'buteo/raster/')
sys.path.append(yellow_path + 'buteo/convolutions/')
...
for tile in tiles_list:
    ...
    dsm_folder = '{}/DSM/{}_TIF_UTM32-ETRS89/'.format(
        base_path, tile.replace('10km', 'DSM'))
    dtm_folder = '{}/DTM/{}_TIF_UTM32-ETRS89/'.format(
        base_path, tile.replace('10km', 'DTM'))
    ...
    for dsm, dtm in zip(dsm_tifs, dtm_tifs):
        ...
        array_to_raster(sobel_npy, reference=dtm,
                        out_path=sobel_path, creation_options=["COMPRESS=LZW"])
        array_to_raster(hat_npy, reference=dtm, out_path=hat_path,
...
ds = gdal.BuildVRT('//pc116900/S Drone div/STENDIGER/vrts/merged.vrt', vrts,
                   options=gdal.BuildVRTOptions(outputBounds=(xmin, ymin, xmax, ymax)))
ds.FlushCache()
...
dtm_list = [str(x) for x in Path(
    '//pc116900/S Drone div/STENDIGER/DTM').rglob('*.tif') if x.name in formatted_onekm]
dsm_list = [str(x) for x in Path(
    '//pc116900/S Drone div/STENDIGER/DSM').rglob('*.tif') if x.name in formatted_onekm]

# %%
dest_sobel_list = [str(x) for x in Path(
    '//pc116900/S Drone div/STENDIGER/SOBEL').rglob('*.tif')]
dest_hat_list = [str(x) for x in Path(
    '//pc116900/S Drone div/STENDIGER/HAT').rglob('*.tif')]


# %%
dtm_list_filtered = [x for x in dtm_list if x.replace(
    'DTM', 'SOBEL') not in dest_sobel_list]
dsm_list_filtered = [x for x in dtm_list if x.replace(
    'DSM', 'SOBEL') not in dest_sobel_list]
...
for dsm, dtm in zip(dsm_list_filtered, dtm_list_filtered):
    ...
    sobel_path = r'//pc116900/S Drone div/STENDIGER/SOBEL/' + \
        os.path.basename(sobel_path)
    hat_path = r'//pc116900/S Drone div/STENDIGER/HAT/' + \
        os.path.basename(hat_path)

    print(sobel_path)
    print(hat_path)

    array_to_raster(sobel_npy, reference=dtm, out_path=sobel_path,
                    creation_options=["COMPRESS=LZW"])
    array_to_raster(hat_npy, reference=dtm, out_path=hat_path,
                    creation_options=["COMPRESS=LZW"])
    print('done with', dtm)
...
sobel_list = [str(x).replace('\\', '/')
              for x in Path('//pc116900/S Drone div/STENDIGER/SOBEL').rglob('*.tif')]
hat_list = [str(x).replace('\\', '/')
            for x in Path('//pc116900/S Drone div/STENDIGER/HAT').rglob('*.tif')]
...
vrts_list = [x for x in glob.glob(
    '//pc116900/S Drone div/STENDIGER/vrts/*.vrt')]
# %%
...
for i, row in buff10gdf.iterrows():
    ...
    gdal.BuildVRT(vrtdir + '10kmnew/' + km10 + '.vrt', km1paths,
                  options=gdal.BuildVRTOptions(outputBounds=extent))
# %%
vrtdir = '//pc116900/S Drone div/STENDIGER/vrts/'
...
for km10name, km1list in nameDict.items():
    ...
    gdal.BuildVRT(vrtdir + '10kmnew/' + km10name + '.vrt', km1paths,
                  options=gdal.BuildVRTOptions(outputBounds=extent))
...
for dtm, dsm, hat, sobel in zip(dtm_tifs, dsm_tifs, hat_tifs, sobel_tifs):
    ...
    gdal.BuildVRT(vrt, [dtm, hat, sobel],
...
dtm_list = [x for x in Path(
    '//pc116900/S Drone div/STENDIGER/DTM').rglob('*.tif')]
...
for tile10km in km10_list:
    ...
    dtm_list_this10km = [str(x)
                         for x in dtm_list if x.name == tile10km + '.tif']
    dsm_list_this10km = [x.replace('DTM', 'DSM').replace(
        'dtm', 'dsm') for x in dtm_list_this10km]
    ...
    for dsm, dtm in zip(dsm_list_this10km, dtm_list_this10km):
        ...
        vrt = vtemp + \
            os.path.basename(dtm).replace('DTM_', '').replace('.tif', '.vrt')
        gdal.BuildVRT(vrt, [dtm, hat_path, sobel_path],
                      options=gdal.BuildVRTOptions(separate=True))
    print('HAT, SOBEL and VRT files created!')

    vrt_list = [vtemp + x + '.vrt' for x in km1_namelist]
    this_10km_geometry = km10_df[km10_df['tilename']
                                 == tile10km]['geometry'].iloc[0]
    print('creating 10km vrt...')

    vrt_10km = gdal.BuildVRT(vtemp + tile10km + '.vrt', vrt_list, options=gdal.BuildVRTOptions(
        outputBounds=getExtent(this_10km_geometry), separate=True))
